...
## 18. 🤝 Multi-Agent Handoff Protocol (Передача между агентами)
* **Context Transfer:** Передача контекста между агентами через JSON-структуру.
* **Role Switching:** Явная передача роли (Planner → Executor → Reviewer).
* **State Serialization:** Сохранение состояния для возобновления работы.
* **Handoff Trigger:** Автоматическое переключение при достижении лимита контекста.
* **Session Summary:** Создание `session_summary.md` при завершении сессии.

## 19. 📊 LLM Evaluation & Quality Assurance
* **LLM-as-a-Judge:** Использование второй модели для оценки качества ответа.
* **Hallucination Detection:** Проверка имен библиотек, версий API, ссылок.
* **Consistency Check:** Сравнение ответа с предыдущими решениями.
* **Bias Audit:** Проверка на предвзятость в рекомендациях.
* **Actionability Score:** Оценка практической применимости совета.

## 20. 💾 Memory Architecture (Архитектура памяти)
* **Short-Term Memory:** Текущий контекст диалога (LangGraph State).
* **Long-Term Memory:** Факты о пользователе/бренде (Mem0, Zep, JSON).
* **Episodic Memory:** История успешных решений для повторного использования.
* **Semantic Memory:** Знания о домене (RAG, Vector DB).
* **Working Memory:** Промежуточные результаты текущей задачи.